...
void lbp::histogram_(const Mat& src, Mat& hist, int numPatterns, bool normed) {
    ...
    if (normed) {
        normalize(hist, hist, 0, 1, NORM_MINMAX, CV_32FC1);
    }
}
...
void lbp::spatial_histogram(const Mat& src, Mat& hist, int numPatterns, const Size& window, int overlap) {
    spatial_histogram(src, hist, numPatterns, window, false, overlap);
}

void lbp::spatial_histogram(const Mat& src, Mat& hist, int numPatterns, const Size& window, bool normed, int overlap) {
	int width = src.cols;
	int height = src.rows;
	...
	for(int x=0; x < width - window.width; x+=(window.width-overlap)) {
		for(int y=0; y < height-window.height; y+=(window.height-overlap)) {
			...
			histograms.push_back(histogram(cell, numPatterns, normed));
		}
	}
    if (normed)
        hist.create(1, histograms.size()*numPatterns, CV_32FC1);
    else
...
// wrappers
void lbp::histogram(const Mat& src, Mat& hist, int numPatterns, bool normed) {
	switch(src.type()) {
		case CV_8SC1: histogram_<char>(src, hist, numPatterns, normed); break;
		case CV_8UC1: histogram_<unsigned char>(src, hist, numPatterns, normed); break;
		case CV_16SC1: histogram_<short>(src, hist, numPatterns, normed); break;
		case CV_16UC1: histogram_<unsigned short>(src, hist, numPatterns, normed); break;
		case CV_32SC1: histogram_<int>(src, hist, numPatterns, normed); break;
	}
}
...
Mat lbp::histogram(const Mat& src, int numPatterns, bool normed) {
	Mat hist;
	histogram(src, hist, numPatterns, normed);
	return hist;
}
